...
# Keep a ratio gate, but avoid flaky failures at the 400-object boundary in
# headless/contended runners where baseline FPS can swing.
UI_RENDER_STRESS_MIN_FPS_RATIO="${UI_RENDER_STRESS_MIN_FPS_RATIO:-0.40}"
...
# Keep this above the 45% FPS-ratio gate to avoid false failures from
# headless browser frame pacing noise while still enforcing stress limits.
UI_RENDER_STRESS_MAX_SMOOTHED_FRAME_MS="${UI_RENDER_STRESS_MAX_SMOOTHED_FRAME_MS:-50}"
...
SCALE_WAVE_BATCH_SIZE="${SCALE_WAVE_BATCH_SIZE:-24}"
SCALE_WAVE_BATCH_DELAY_MS="${SCALE_WAVE_BATCH_DELAY_MS:-500}"
...
run_step "Multi-client scale benchmark" \
  node "$ROOT_DIR/scripts/ui_bench/multi_client.js" \
    ...
    --wave-batch-size "$SCALE_WAVE_BATCH_SIZE" \
    --wave-batch-delay-ms "$SCALE_WAVE_BATCH_DELAY_MS" \
