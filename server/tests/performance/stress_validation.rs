...
fn queue_random_input(server: &MassiveGameServer, player_id: &PlayerID, sequence: u32) {
    let input = PlayerInputData {
        ...
        sequence,
        move_forward: sequence % 3 == 0,
        move_backward: sequence % 5 == 0,
        move_left: sequence % 7 == 0,
        move_right: sequence % 11 == 0,
        shooting: sequence % 4 == 0,
        reload: sequence % 20 == 0,
        rotation: (sequence as f32 * 0.1) % std::f32::consts::TAU,
        melee_attack: sequence % 30 == 0,
        use_ability_slot: 0,
