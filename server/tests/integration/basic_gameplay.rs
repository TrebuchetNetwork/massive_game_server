...
fn create_destructible_wall(
    ...
    if let Some(partition) = server.world_partition_manager.get_partition(partition_idx) {
        partition
            ...
            .insert(wall.id, wall.clone());
        info!(
            "[Test Wall] Added wall ID {} to partition {}",
            wall_id, partition_idx
        );
    } else {
        panic!(
            "[Test Wall] Test setup error: Could not find partition {} to add wall {}.",
            partition_idx, wall_id
        );
    }
    server.wall_respawn_manager.register_wall(&wall);
    info!(
        "[Test Wall] Created and registered wall ID {} at ({}, {}) with health {}",
        wall_id, x, y, health
    );
    wall_id
}
    ...
    if server
        ...
        .get_player_state(
            &server
                .player_manager
                .id_pool
                .get_or_create(owner_player_id_str),
        )
        .is_none()
    {
        info!(
            "[Test Projectile] Owner {} not in PlayerManager, adding for test.",
            owner_player_id_str
        );
        server.player_manager.add_player(
            owner_player_id_str.to_string(),
            owner_player_id_str.to_string(),
            0.0,
            0.0,
        );
...
fn process_projectile_wall_collision(
    ...
    info!(
        "[Test Collision] Processing collision: projectile ID {} vs wall ID {}",
        projectile.id, wall_id
    );
    let mut wall_found_and_processed = false;
    for partition_arc in server
        .world_partition_manager
        .get_partitions_for_processing()
    {
        if let Some(mut wall_entry) = partition_arc.all_walls_in_partition.get_mut(&wall_id) {
            ...
            info!(
                "[Test Collision] Wall ID {} found in partition {}. Current health: {}",
                wall_id, partition_arc.id, wall_mut.current_health
            );
    ...
    if !wall_found_and_processed {
        panic!(
            "[Test Collision] Test error: Wall ID {} not found for collision processing.",
            wall_id
        );
    }
}
...
fn test_wall_creation_and_registration() {
    ...
    assert!(
        wall_exists_in_partition,
        "Wall was not added to a partition."
    );
    ...
    assert!(
        server
            ...
            .is_wall_template_registered_for_test(wall_id),
        "Wall template not registered in WallRespawnManager."
    );
    info!(
        "[Test Result] test_wall_creation_and_registration: Wall template registration verified."
    );
}
...
fn test_wall_destruction_and_respawn_scheduling() {
    ...
    server.player_manager.add_player(
        player_owner_id.to_string(),
        player_owner_id.to_string(),
        0.0,
        0.0,
    );

    let projectile1 = create_projectile(
        &server,
        player_owner_id,
    ...
    let partition_idx = server
        .world_partition_manager
        .get_partition_index_for_point(150.0 + 10.0, 150.0 + 10.0);
    if let Some(partition) = server.world_partition_manager.get_partition(partition_idx) {
    ...
    assert_eq!(
        final_health, 0,
        "Wall health was not reduced to 0 after destruction. Current health: {}",
        final_health
    );
    ...
    assert!(
        respawn_timer_opt.is_some(),
        "Wall was not scheduled for respawn after destruction."
    );
    if let Some(timer) = respawn_timer_opt {
        assert!(
            timer.as_secs() >= 29 && timer.as_secs() <= 91,
            "Wall respawn timer ({}s) is out of expected range (30-90s).",
            timer.as_secs()
        );
        info!("[Test Result] test_wall_destruction_and_respawn_scheduling: Wall respawn timer is {:?}.", timer);
    }

    let projectile2 = create_projectile(
        &server,
        "player_hitter_destroyed",
    ...
    assert_eq!(
        health_after_second_hit, 0,
        "Wall health changed after being hit while already destroyed."
    );
    info!("[Test Result] test_wall_destruction_and_respawn_scheduling: Wall health remains 0 after second hit.");

    let respawn_timer_after_second_hit_opt =
        server.wall_respawn_manager.get_wall_respawn_timer(wall_id);
    assert!(
        respawn_timer_after_second_hit_opt.is_some(),
        "Respawn timer disappeared after second hit on destroyed wall."
    );
...
fn test_wall_destruction_scenario() {
    ...
    server
        .player_manager
        .add_player(player1_id.to_string(), player1_id.to_string(), 0.0, 0.0);
    server
        .player_manager
        .add_player(player2_id.to_string(), player2_id.to_string(), 0.0, 0.0);
    server
        .player_manager
        .add_player(player3_id.to_string(), player3_id.to_string(), 0.0, 0.0);
    ...
    assert_eq!(
        ...
        "Projectile1 damage mismatch for Rifle (expected 10)."
    );
    process_projectile_wall_collision(&server, &projectile1, wall_id);
    ...
    assert_eq!(
        health_after_hit1, 90,
        "Wall health after first rifle hit is incorrect (expected 90)."
    );
    info!(
        "[Test Result] test_wall_destruction_scenario: Health after Rifle hit: {}",
        health_after_hit1
    );
    ...
    assert_eq!(
        projectile2.damage, 50,
        "Projectile2 damage mismatch for Sniper (expected 50)."
    );
    process_projectile_wall_collision(&server, &projectile2, wall_id);
    ...
    assert_eq!(
        health_after_hit2, 40,
        "Wall health after sniper hit is incorrect (expected 40)."
    );
    info!(
        "[Test Result] test_wall_destruction_scenario: Health after Sniper hit: {}",
        health_after_hit2
    );

    let projectile3 = create_projectile(
        &server,
        player3_id,
        ServerWeaponType::Sniper,
        90.0,
    ...
    assert_eq!(
        projectile3.damage, 50,
        "Projectile3 damage mismatch for Sniper (expected 50)."
    );
    ...
    assert_eq!(
        health_after_destruction, 0,
        "Wall health should be 0 after final hit."
    );
    info!(
        "[Test Result] test_wall_destruction_scenario: Health after final Sniper hit: {}",
        health_after_destruction
    );
    ...
    assert!(
        server
            ...
            .is_some(),
        "Destroyed wall not scheduled for respawn."
    );
...
async fn test_player_spawning_and_movement() {
    ...
    for i in 0..num_spawn_attempts {
        ...
        // Refresh the current wall state for each spawn attempt.
        let all_current_walls: Vec<Wall> = server.collect_all_walls_current_state();
        ...
        info!(
            "[Spawn Test Iteration {}] Suggested spawn at ({:.2}, {:.2}) for player {}",
            i, spawn_pos.x, spawn_pos.y, player_id_str
        );
        ...
        let initial_player_state_snapshot = server
            .player_manager
        ...
        if let Some(colliding_wall_id) = is_player_colliding_with_wall(
            initial_player_state_snapshot.x,
            initial_player_state_snapshot.y,
            &all_current_walls_after_spawn,
        ) {
...

        {
            ...
            let input = PlayerInputData {
                timestamp: SystemTime::now()
                    ...
                    .unwrap()
                    .as_millis() as u64, // More realistic timestamp
        ...
        server.synchronize_state(true).await; // Ensure spatial index, etc., are updated

        let final_player_state_snapshot = server
            .player_manager
            .get_player_state(&player_id_arc)
            .unwrap()
            .clone();
        info!(
            "[Spawn Test Iteration {}] Player {} final pos: ({:.2}, {:.2})",
            i, player_id_str, final_player_state_snapshot.x, final_player_state_snapshot.y
        );
        ...
        if !pos_changed {
            ...
            let point_in_front = Vec2::new(
                initial_x + intended_move_dir.x * (PLAYER_RADIUS + 5.0),
                initial_y + intended_move_dir.y * (PLAYER_RADIUS + 5.0),
            );

            let mut very_near_wall_in_front = false;
            if let Some(_wall_id) = is_player_colliding_with_wall(
                point_in_front.x,
                point_in_front.y,
                &all_current_walls_after_spawn,
            ) {
            ...
            info!(
                "[Spawn Test Iteration {}] Player {} moved successfully.",
                i, player_id_str
            );
        }

        // Final check: ensure player is not stuck inside a wall after movement
        if let Some(colliding_wall_id_after_move) = is_player_colliding_with_wall(
            final_player_state_snapshot.x,
            final_player_state_snapshot.y,
            &all_current_walls_after_spawn,
        ) {
        ...
        info!(
            "[Spawn Test Iteration {}] Player {} final position is clear of walls.",
            i, player_id_str
        );
