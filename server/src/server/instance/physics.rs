...
impl MassiveGameServer {
    ...
    fn invalidate_structural_wall_cache(&self) {
        if let Some(cache) = CACHED_WALLS.get() {
            cache.write().0 = u64::MAX;
        }
    }

    fn resolve_progressive_parent_wall_id(&self, wall_id: EntityId) -> EntityId {
    ...
    fn clear_progressive_fragments_for_parent(&self, parent_wall_id: EntityId) -> Vec<EntityId> {
        if !self.progressive_destructible_enabled {
            return Vec::new();
        }
    ...
    fn build_progressive_fragment_walls(parent_wall: &Wall, stage: u8) -> Vec<Wall> {
    ...
    fn apply_progressive_wall_fragmentation(
        &self,
        parent_wall: &Wall,
        target_stage: u8,
