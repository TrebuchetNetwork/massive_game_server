...
const TACTICAL_POSITIONS: [(f32, f32); 12] = [
    ...
    (500.0, 500.0),
    (0.0, -700.0),
    (0.0, 700.0), // North/South positions
    (-700.0, 0.0),
    (700.0, 0.0), // East/West positions
    (-350.0, -350.0),
    (350.0, -350.0), // Inner positions
    (-350.0, 350.0),
    (350.0, 350.0),
];
...
impl BotAISystem {
        ...
        for bot_id_arc in bot_ids {
            ...
            if let Some(bot_current_state) = bot_current_state_owned_opt {
                if !bot_current_state.alive {
                    if let Some(mut bot_controller_entry) =
                        server_instance.bot_players.get_mut(&bot_id_arc)
                    {
                        ...
                        if bot_controller.behavior_state != BotBehaviorState::Idle {
                            debug!(
                                "[Bot {} ({})]: Died. Resetting to Idle.",
                                bot_current_state.username,
                                bot_id_arc.as_str()
                            );
                        }
                    ...
                    server_instance.bot_players.get_mut(&bot_id_arc)
                {
                    ...
                    if bot_controller.target_position.is_some()
                        && (bot_controller.current_path.is_empty()
                            || current_time_instant
                                .duration_since(bot_controller.path_recalculation_timer)
                                > BOT_PATH_RECALCULATION_INTERVAL)
                    {
                    ...
                    let mut input = PlayerInputData {
                        ...
                        shooting: false,
                        reload: false,
                        rotation: bot_current_state.rotation,
                        melee_attack: false,
                    ...
                    if let Some(mut actual_bot_player_state_entry) = server_instance
                        .player_manager
                        .get_player_state_mut(&bot_id_arc)
                    {
    ...
    fn make_tactical_decision(
        ...
        if is_ctf && bot_state.team_id != 0 {
            ...
            if bot_state.is_carrying_flag_team_id != 0
                && bot_state.is_carrying_flag_team_id != bot_state.team_id
            {
                ...
                bot_controller.current_path.clear();
                debug!(
                    "[Bot {} ({})]: Carrying enemy flag! Rushing to base at ({:.1}, {:.1})",
            ...
            for entry in server.bot_players.iter() {
                ...
                if let Some(bot_player_state) = all_player_entities.get(bot_id) {
                    if bot_player_state.team_id == bot_state.team_id && bot_player_state.alive {
                        ...
                        if let (Some(target_pos), Some(flag_pos)) =
                            (bot_ctrl.target_position, enemy_flag_pos)
                        {
                            let dist_to_flag = ((target_pos.x - flag_pos.x).powi(2)
                                + (target_pos.y - flag_pos.y).powi(2))
                            .sqrt();
                            if dist_to_flag < 100.0
                                && bot_ctrl.behavior_state == BotBehaviorState::MovingToObjective
                            {
            ...
            if let Some(enemy_flag_state) = ctf_flag_states.get(&enemy_team) {
                if (enemy_flag_state.status == fb::FlagStatus::AtBase
                    || enemy_flag_state.status == fb::FlagStatus::Dropped)
                    && should_go_for_flag
                {
                    bot_controller.behavior_state = BotBehaviorState::MovingToObjective;
            ...
            if let Some(our_flag_state) = ctf_flag_states.get(&bot_state.team_id) {
                if our_flag_state.status == fb::FlagStatus::Carried && rng.gen_bool(0.6) {
                    // 60% chance to hunt flag carrier
                    ...
                    for (player_id, player_state) in all_player_entities {
                        if player_state.is_carrying_flag_team_id == bot_state.team_id {
                            ...
                            bot_controller.target_position =
                                Some(Vec2::new(player_state.x, player_state.y));
                            bot_controller.current_path.clear();
                            debug!(
                                "[Bot {} ({})]: Hunting enemy flag carrier {} at ({:.1}, {:.1})",
                                bot_state.username,
                                bot_id_str,
        ...
        if needs_health {
            ...
            if let Some((pickup_pos, _)) =
                Self::find_nearest_health_pickup(bot_state, active_pickups)
            {
                ...
                debug!(
                    "[Bot {} ({})]: Low health ({}), seeking health pickup at ({:.1}, {:.1})",
        ...
        let nearby_player_ids =
            server
                .spatial_index
                .query_nearby_players(bot_state.x, bot_state.y, scan_radius);
        ...
        for entity_id_arc in nearby_player_ids {
            if let Some(other_state) = all_player_entities.get(&entity_id_arc) {
                ...
                if is_enemy {
                    let dist = ((other_state.x - bot_state.x).powi(2)
                        + (other_state.y - bot_state.y).powi(2))
                    .sqrt();
        ...
        if let Some((closest_enemy, dist)) = enemies.first() {
            ...
            if *dist < weapon_range && !needs_health {
                ...
                let flank_pos = Self::calculate_flank_position(
                    bot_pos,
                    Vec2::new(closest_enemy.x, closest_enemy.y),
                    rng,
                );
                ...
                debug!(
                    "[Bot {} ({})]: Flanking {} via ({:.1}, {:.1})",
                    bot_state.username,
                    bot_id_str,
                    closest_enemy.username,
                    flank_pos.x,
                    flank_pos.y
                );
    ...
    fn choose_tactical_position(
        bot_pos: Vec2,
        enemies: &[(&PlayerState, f32)],
        rng: &mut impl Rng,
    ) -> Vec2 {
        ...
        if !enemies.is_empty() {
            ...
            let enemy_center = (
                enemy_center.0 / enemies.len() as f32,
                enemy_center.1 / enemies.len() as f32,
            );
            ...
            for &pos in &TACTICAL_POSITIONS {
                let dist_to_pos =
                    ((pos.0 - bot_pos.x).powi(2) + (pos.1 - bot_pos.y).powi(2)).sqrt();
                let dist_to_enemy =
                    ((pos.0 - enemy_center.0).powi(2) + (pos.1 - enemy_center.1).powi(2)).sqrt();
    ...
    fn generate_warzone_input(
        ...
        let current_movement_target: Option<Vec2> = if !bot_controller.current_path.is_empty() {
            ...
            let random_target = Vec2::new(
                (bot_state.x + random_dist * random_angle.cos())
                ...
                if let Some(enemy_state) = all_player_entities.get(enemy_id_arc) {
                    if enemy_state.alive {
                        ...
                        let projectile_speed =
                            Self::get_projectile_speed(bot_state.weapon).max(1.0);
                        let time_to_target = dist_to_enemy / projectile_speed;

                        let predicted_x =
                            enemy_state.x + enemy_state.velocity_x * time_to_target * 0.8;
                        let predicted_y =
                            enemy_state.y + enemy_state.velocity_y * time_to_target * 0.8;

                        input.rotation =
                            (predicted_y - bot_state.y).atan2(predicted_x - bot_state.x);
    ...
    fn line_intersects_line(l1p1: Vec2, l1p2: Vec2, l2p1: Vec2, l2p2: Vec2) -> bool {
        ...
        let t =
            ((l1p1.x - l2p1.x) * (l2p1.y - l2p2.y) - (l1p1.y - l2p1.y) * (l2p1.x - l2p2.x)) / den;
        let u =
