...
impl AsyncBotAI {
    ...
    pub fn start_bot_ai_task(
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                // Get all bot IDs
                let bot_ids: Vec<PlayerID> = server
                    .bot_players
                    .iter()
                ...
                for bot_id in bot_ids {
                    ...
                    let should_think = bot_think_times
                        .get(&bot_id)
                        .map(|&last_think| {
                            now.duration_since(last_think) > Duration::from_millis(500)
                        })
                        .unwrap_or(true);
                    ...
                    if should_think {
                        ...
                        let (bot_state, bot_controller) = {
                            let bot_state_opt = server
                                .player_manager
                                .get_player_state(&bot_id)
                                .map(|guard| (*guard).clone());

                            let bot_controller_opt = server
                                .bot_players
                                .get(&bot_id)
                        ...
                        let decision =
                            Self::make_intelligent_decision(&server, &bot_state, &bot_controller)
                                .await;
    ...
    async fn make_intelligent_decision(
        ...
        if !incoming_projectiles.is_empty() {
            ...
            if let Some(proj) = incoming_projectiles.first() {
                let dodge_angle = (proj.y - bot_state.y).atan2(proj.x - bot_state.x)
        ...
        // Priority 2: Engage nearby enemies
            ...
            let dist_to_enemy =
                ((enemy_pos.x - bot_state.x).powi(2) + (enemy_pos.y - bot_state.y).powi(2)).sqrt();
        ...
        // Priority 3: Seek health/ammo if needed
            ...
            // Find best pickup
            let best_pickup = nearby_pickups.iter().min_by_key(|(_, pos, pickup_type)| {
        ...
        if let Some(mut bot_controller_entry) =
            server.bot_players.get_mut(&bot_controller.player_id)
        {
    ...
    fn find_nearby_enemies(
        ...
        let nearby_players =
            server
                .spatial_index
                .query_nearby_players(bot_state.x, bot_state.y, range);
    ...
    fn detect_incoming_projectiles(
        ...
        for proj in projectiles_guard.iter() {
            ...
            if dist_sq <= range * range {
                ...
                let future_dist_sq =
                    (future_x - bot_state.x).powi(2) + (future_y - bot_state.y).powi(2);
    ...
    fn generate_movement_input(
        ...
        if let Some(target) = target_pos {
            ...
            if dist_sq > 25.0 {
                // Move if more than 5 units away
                let angle_diff = ((input.rotation - bot_state.rotation)
    ...
    pub fn poll_and_apply_decisions(&mut self, server: &MassiveGameServer) {
        while let Ok(decision) = self.decision_receiver.try_recv() {
            if let Some(mut player_state_entry) =
                server.player_manager.get_player_state_mut(&decision.bot_id)
            {
